class LiveStreamSeriesController < ApplicationController
  ...
  def by_band
    unless ( params[:id] && (@band = Band.includes(:live_stream_series).find(params[:id])) )
      ...
      # end
      @live_stream_series = @band.live_stream_series.where(:public => true).includes(:streamapi_streams).order('streamapi_streams.starts_at ASC')
    end
        ...
        :streams => series.streamapi_streams.collect{ |stream|
          if stream.public
            theme = StreamapiStreamTheme.find(stream.viewer_theme_id)
            {
              ...
              :start_time => stream.starts_at.strftime('%I:%M%p %Z'),
