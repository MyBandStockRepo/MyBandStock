...
=begin
    if @user = User.find_by_id(session[:user_id])
      #RAILS 3
      @number_of_unopened_mail = @user.band_mails.where(['opened = ?', false]).size
...
=end
  ...
  def band_home
    ...
    if @user = User.find_by_id(session[:user_id])
      ...
      #RAILS 3
      @bands = @user.associations.where({:associations => {:name => ['admin', 'member']}}, :joins => :band, :group => 'band_id').collect{|a| a.band}
    end
=begin
