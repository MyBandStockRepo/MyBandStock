...
scheduler.every '1h' do
  ...
  Rails.logger.info '[AUTO_EMAIL_SCHEDULER]['+Time.now.to_s+'] running script'
  ...
  for stream in streamapi_streams
    ...
    Rails.logger.info '[AUTO_EMAIL_SCHEDULER]['+Time.now.to_s+'] Found a stream that needs to send out an email STREAM_ID='+stream.id.to_s
    ...
    if email_return_val == true
      ...
      stream.save()
      Rails.logger.info '[AUTO_EMAIL_SCHEDULER]['+Time.now.to_s+']  ==> Mail has been put onto the delayed job queue for all users.'
    elsif email_return_val == false
      #failed for everyone
      #users_have_been_notified already false, don't need to do anything
      Rails.logger.info '[AUTO_EMAIL_SCHEDULER]['+Time.now.to_s+']  ==> ERROR: No mail was sent out, either nobody was added to the queue, or there was another error in the method.'      
    else
      #failed for some, worked for some, pretend like it worked for everyone... it's only a reminder, not vital information
      stream.users_have_been_notified = true
      stream.save()
      Rails.logger.info '[AUTO_EMAIL_SCHEDULER]['+Time.now.to_s+']  ==> ERROR: Some users were added to the job queue, but not all.'      
    end
  end
  Rails.logger.info '[AUTO_EMAIL_SCHEDULER]['+Time.now.to_s+'] finishing script'
end
