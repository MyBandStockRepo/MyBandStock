  ...
  function main() { 
    ...
    { 
      ...
      var band_id = mybandstockArtistID;
            var url_host = secure_mbs_source_url+"/bands/"; // AJAX requests with user secrets should be send 
over the secure URL.
      ...
      // Initial Bar State
      var mbs_initial_bar_state = "<div class=\"mbs-bar-branding\"></div><div 
class=\"mbs-bar-login-wrapper\"><div class=\"mbs-bar-login\"><span class=\"mbs-email\">Email: <input 
id=\"mbs_user_email\" name=\"user[email]\" size=\"30\" type=\"text\" /></span></div><div 
class=\"clear\"></div><input id=\"mbs_user_submit\" name=\"commit\" type=\"submit\" value=\"Submit\" /><span 
class=\"mbs-cancel-this\" style=\"display:none;\"><a href=\""+ current_url +"\" 
title=\"cancel\">cancel</a></span></div><div class=\"mbs-bar-instructions\">Start earning rewards today!<br 
/>Enter an email address to login or sign-up.</div>";
      ...
      jQuery('body').append('<div id="mbs-bar-message-box" class="mbs-alpha80" style="display:none;"><span 
id="mbs-bar-notification"></span><a id=\"mbs-bar-close-notifications\">X</a></div>');
            
      // Rewards buttons
      var rewards_buttons = "<div class=\"mbs-points-containers\"><span class=\"mbs-earn-points\"><a 
id=\"mbs-ways-to-earn-link\" onClick=\"mybandstockToggleWaysToEarn()\">Get Bandstock</a></span><span 
class=\"mbs-rewards\"><a id=\"mbs-rewards-link\" onClick=\"mybandstockToggleRewardsDiv()\">View 
Levels</a></span></div>";
            
      // check to see if there's a cookie set, if there is, ping the server to find the user, if not, render the 
login
      if (jQuery.cookie('_mbs'))
      { 
        var salt = jQuery.cookie("_mbs");
        var jsonp_url = url_host + band_id + "/shareholders.json?callback=?&salt=" + salt;
        jQuery.getJSON(jsonp_url, function(data) { // send the params to the app and append the response to the 
main container
          jQuery('#js-bar-container').html(data.html);
          jQuery('#js-bar-container').append(rewards_buttons);
          jQuery('#js-bar-container').append("<div class=\"mbs-bar-login-wrapper\"><span 
class=\"mbs-logout-link\"><a onClick=\"mybandstock_log_user_out()\"> Logout</a></span></div>");         
        });
      } else {
        //login stuff
      ...
      jQuery('#js-bar-container .mbs-cancel-this').click(function() {
        ...
        jQuery('.mbs-bar-login').html("<span class=\"mbs-email\">Email: <input id=\"mbs_user_email\" 
name=\"user[email]\" size=\"30\" type=\"text\" /></span>");       
        return false;
      });
      ...
      jQuery('#js-bar-container #mbs_user_submit').click(function() {
        ...
        var email = jQuery('#js-bar-container input#mbs_user_email').val(); //capture the email entered
                var email_confirmation = jQuery('#js-bar-container 
input#mbs_user_email_confirmation').val();//capture the email entered if new user
                var pass = jQuery('#js-bar-container input#mbs_user_password').val(); //capture the password 
entered
        ...
        if(email != null)
          ...
          var jsonp_url = url_host + band_id + "/shareholders.json?callback=?&email=" + email + "&password=" + 
pass + "&email_confirmation=" + email_confirmation + "&first_name=" + first_name; //pass those params to the 
query string
        ...
        jQuery.getJSON(jsonp_url, function(data) {
          ...
          // BAR STATES
          // Log the user in
                if (data.msg && data.msg != "delete" && data.msg != "need-password" && data.msg != 
"create-new-user" && data.msg != "user-error"){ //if the app sent a message that is not delete, we set a cookie, 
log in the user and remove the submit button
...
class=\"mbs-logout-link\"><a onClick=\"mybandstock_log_user_out()\"> Logout</a></span></div>");                     
          ...
          // Need to delete user cookie
                 else if (data.msg && data.msg == "delete"){//if the app sent a message of 'delete'(the user 
couldn't be found from the cookie info), we reset the cookie
